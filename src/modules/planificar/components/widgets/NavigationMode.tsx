...
function TransitNavigation({ prediction, onExit }: NavigationModeProps) {
      ...
      (pos) => {
        ...
        for (let i = currentStopIdx + 1; i < stations.length; i++) {
          ...
          if (i - 1 < segments.length && segments[i - 1]?.coordinates?.length > 0) {
            const lastCoord = segments[i - 1].coordinates[segments[i - 1].coordinates.length - 1];
  ...
  const progress = stations.length > 1 ? (currentStopIdx / (stations.length - 1)) * 100 : 0;
  const remainingTime = totalTime * (1 - progress / 100);

  const modeIcon = mode === "transmilenio" ? <Train size={18} /> : <Bus size={18} />;
  const modeColor = mode === "transmilenio" ? "text-red-500" : "text-blue-500";
  const modeBg = mode === "transmilenio" ? "bg-red-500" : "bg-blue-500";
  const modeBgLight = mode === "transmilenio" ? "bg-red-500/10" : "bg-blue-500/10";
  ...
  return (
    <div className="fixed inset-0 z-[700] flex flex-col bg-background">
      {/* Header */}
      <div className={`${modeBg} text-white px-4 py-3 flex items-center gap-3 shadow-lg z-10`}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        {/* ETA card */}
        <div className={`${modeBgLight} rounded-xl p-3 mb-3 flex items-center justify-between`}>
          <div className="flex items-center gap-2">
            <Clock size={14} className={modeColor} />
            <span className="text-xs font-medium">Llegas aprox.</span>
          </div>
          <div className="text-right">
            <span className="text-sm font-bold">{formatETA(remainingTime * 60)}</span>
            <span className="text-[10px] text-default-400 ml-2">
              {formatMinutes(remainingTime)} · {formatDistance(totalDist * 1000 * (1 - progress / 100))}
            </span>
          </div>
        </div>
              ...
              <div key={`nav-stop-${i}`} className="flex items-stretch gap-3">
                ...
                <div className="flex flex-col items-center w-5 shrink-0">
                  {isFirst ? (
                    ...
                    <div className={`w-4 h-4 rounded-full ${modeBg} border-2 border-white shadow-lg shrink-0 mt-2.5 animate-pulse`} />
                  ) : isPast ? (
                  ...
                  {!isLast && (
                    <div className={`w-0.5 flex-1 min-h-[20px] ${isPast ? "bg-default-300" : modeBg + "/30"}`} />
                  )}
                </div>

                {/* Station content */}
                <div className={`flex-1 pb-2 pt-1.5 ${isCurrent ? "scale-[1.02]" : ""}`}>
                  ...
                  {isCurrent && !isFirst && !isLast && (
                    <p className={`text-[10px] ${modeColor} mt-0.5 font-medium`}>
                      ← Estás aquí
                    </p>
                  )}
                  {isLast && (
      ...
      <div className="px-4 py-3 bg-background border-t border-divider flex items-center gap-3 shrink-0">
        ...
        <div className={`w-11 h-11 rounded-full ${modeBgLight} border border-divider flex items-center justify-center ${modeColor}`}>
          {modeIcon}
        </div>
      </div>
    </div>
...
function VehicleNavigation({ prediction, onExit }: NavigationModeProps) {
  ...
  const steps: NavStep[] = useMemo(() => prediction.navigation_steps || [], [prediction]);
  ...
  const getStepCoord = useCallback((idx: number): [number, number] | null => {
    const segments = prediction.risk_segments || [];
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        if (routeCoords.length > 0) {
          const minDist = Math.min(...routeCoords.map((c) => haversineM(lat, lng, c[0], c[1])));
    ...
    return (
      <div className="fixed inset-0 z-[700] bg-background flex items-center justify-center">
        <div className="text-center p-6">
          ...
          <p className="text-sm text-default-500 mb-2">Ruta calculada sin instrucciones detalladas.</p>
          <p className="text-xs text-default-400 mb-4">
            {formatDistance(prediction.total_distance_km * 1000)} · {formatMinutes(prediction.total_time_minutes)}
          </p>
  ...
  return (
    <div className="fixed inset-0 z-[700] flex flex-col bg-background">
      ...
      <div className="flex-1 relative overflow-hidden">
        <MapContainer center={mapCenter} zoom={17} zoomControl={false} attributionControl={false} style={{ height: "100%", width: "100%" }}>
          <TileLayer url="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png" />
          {userPos && <FollowUser lat={userPos.lat} lng={userPos.lng} heading={heading} />}
          {routeCoords.length > 0 && (
            <Polyline positions={routeCoords} pathOptions={{ color: "#10b981", weight: 6, opacity: 0.9 }} />
          )}
          {userPos && (
      ...
      {nextStep && (
        <div className="px-4 py-2.5 bg-default-50 border-t border-divider flex items-center gap-2">
          ...
          <p className="text-xs text-default-600 truncate flex-1">{nextStep.instruction}</p>
          <span className="text-xs text-default-400 font-medium shrink-0">{formatDistance(nextStep.distance_m)}</span>
      ...
      <div className="px-4 py-3 bg-background border-t border-divider flex items-center gap-3">
        <button onClick={onExit} className="w-11 h-11 rounded-full bg-danger/10 border border-danger/20 flex items-center justify-center text-danger active:scale-90">
          <X size={20} />
        </button>
        <div className="flex-1 text-center">
          <p className="text-lg font-bold text-foreground">{formatETA(remaining.time)}</p>
          <p className="text-[10px] text-default-400">{formatDistance(remaining.distance)} • {Math.round(remaining.time / 60)} min</p>
        </div>
