...
export function UsersTab() {
  ...
  const [userPerms, setUserPerms] = useState<{ rolePermissions: Permission[]; extraPermissions: Permission[] } | null>(null);
  ...
  const startEditing = () => {
    ...
    if (userPerms) setDraftExtraPerms(userPerms.extraPermissions.map((p) => p.id));
    setEditing(true);
  };
  ...
  const saveChanges = async () => {
    ...
    if (draftActive !== selectedUser.isActive) {
      await fetch(`/api/admin/users/${selectedUser.id}/status`, { method: "PATCH" });
    }
    // Save extra perms
    ...
    setSelectedUser((u) => u ? { ...u, roleId: draftRoleId, isActive: draftActive, role: roles.find((r) => r.id === draftRoleId)! } : null);
  };
    ...
    <>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        <div className="w-[120px]">
          ...
          <Select aria-label="Filtro de estado"
            size="sm"
            variant="bordered"
        ...
        {filtered.map((user) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[9px] text-default-400 truncate">{user.email}</p>
            </div>
            <span className="text-[9px] px-2 py-0.5 rounded-full bg-default-100 capitalize hidden md:inline">
              {user.role?.name}
            </span>
            <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-medium ${user.isActive ? "bg-success/20 text-success" : "bg-danger/20 text-danger"}`}>
      ...
      {selectedUser && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center">
          <button type="button" className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setSelectedUser(null)} aria-label="Cerrar" />
          <div className="relative w-full max-w-lg mx-4 bg-background border border-divider rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-divider shrink-0">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-[12px] font-semibold">{selectedUser.name}</p>
                  <p className="text-[10px] text-default-400">{selectedUser.email}</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                {!editing && (
                  <button onClick={startEditing} className="p-1.5 rounded-lg hover:bg-default-100 text-default-400">
                    <Edit2 size={14} />
                  </button>
                )}
                <button onClick={() => setSelectedUser(null)} className="p-1.5 rounded-lg hover:bg-default-100 text-default-400">
                  <X size={14} />
                </button>
              </div>
            </div>
                ...
                <div>
                  ...
                  {editing ? (
                    <Select aria-label="Filtro de estado"
                      ...
                      classNames={{ trigger: "h-8 min-h-8", value: "text-[11px]" }}
                      items={roles.map((r) => ({ id: String(r.id), name: r.name }))}
                    >
                      {(item) => <SelectItem key={item.id}>{item.name}</SelectItem>}
                    </Select>
                  ) : (
                    <p className="text-[11px] font-medium capitalize">{selectedUser.role?.name}</p>
                  )}
                </div>
                <div>
                  ...
                  {editing ? (
                    <Select aria-label="Filtro de estado"
                      ...
                      items={[{ id: "active", name: "Activo" }, { id: "inactive", name: "Inactivo" }]}
                    >
                      {(item) => <SelectItem key={item.id}>{item.name}</SelectItem>}
                    </Select>
                  ) : (
                    <p className={`text-[11px] font-medium ${selectedUser.isActive ? "text-success" : "text-danger"}`}>
                      {selectedUser.isActive ? "Activo" : "Inactivo"}
                    </p>
                  )}
                </div>
                <div>
                  <p className="text-[9px] text-default-400 mb-1">Miembro desde</p>
                  <p className="text-[11px]">{new Date(selectedUser.createdAt).toLocaleDateString("es-CO")}</p>
                </div>
              </div>
              ...
              {userPerms && (
                <div>
                  <p className="text-[10px] font-semibold text-default-500 mb-2 flex items-center gap-1">
                    ...
                    {editing && allPerms.some((p) => !userPerms.rolePermissions.some((rp) => rp.id === p.id) && !draftExtraPerms.includes(p.id)) && (
                      <span className="text-[8px] text-primary font-normal ml-1">(click para agregar extras)</span>
                    )}
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                    {allPerms.map((perm) => {
                      const fromRole = userPerms.rolePermissions.some((p) => p.id === perm.id);
                      ...
                      return (
                        <label key={perm.id} className="flex items-center gap-2 text-[10px] px-2 py-1.5 rounded-lg hover:bg-default-50">
                          ...
                          />
                          <span className={fromRole ? "text-default-400" : isExtra ? "text-primary font-medium" : ""}>
                            {perm.description || perm.key}
                          </span>
                          {fromRole && <span className="text-[7px] text-default-300 ml-auto">(rol)</span>}
                          {isExtra && <span className="text-[7px] text-primary ml-auto">(extra)</span>}
                        </label>
                      );
                    })}
                  </div>
