...
export function PermissionsTab({
  ...
  const load = () => {
    fetch("/api/admin/permissions").then((r) => (r.ok ? r.json() : [])).then(setPermissions);
  };
  ...
  const create = async () => {
    ...
    await fetch("/api/admin/permissions", {
      ...
      body: JSON.stringify({ module: module.trim(), action: action.trim(), description: description.trim() || undefined }),
    });
    setModule("");
    setAction("");
    setDescription("");
  ...
  const filtered = permissions.filter((p) => {
    if (search && !p.key.toLowerCase().includes(search.toLowerCase()) && !(p.description || "").toLowerCase().includes(search.toLowerCase())) return false;
    ...
    <>
        ...
        {modules.map((mod) => (
          <div key={mod}>
            ...
            <div className="space-y-1.5">
              {filtered.filter((p) => p.module === mod).map((perm) => (
                <div
                  key={perm.id}
                  onClick={() => { setSelectedPerm(perm); setEditingPerm(false); setDraftDesc(perm.description || ""); }}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-medium text-foreground">{perm.description || perm.key}</p>
      ...
      {selectedPerm && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center">
          <button type="button" className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setSelectedPerm(null)} aria-label="Cerrar" />
          <div className="relative w-full max-w-sm mx-4 bg-background border border-divider rounded-2xl shadow-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[12px] font-semibold">{selectedPerm.key}</p>
                  <p className="text-[9px] text-default-400">{selectedPerm.module} → {selectedPerm.action}</p>
                </div>
              </div>
              <button onClick={() => setSelectedPerm(null)} className="p-1 rounded hover:bg-default-100 text-default-400"><X size={14} /></button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                {editingPerm ? (
                  ...
                  <p className="text-[11px] text-foreground">{selectedPerm.description || "Sin descripción"}</p>
                )}
              </div>
              <div>
                <p className="text-[9px] font-medium text-default-500 mb-1">{t("admin.module")}</p>
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-divider">
              ...
              {editingPerm ? (
                <div className="flex gap-2">
                  <button onClick={() => setEditingPerm(false)} className="px-3 py-1.5 rounded-lg text-[10px] font-medium text-default-500 border border-divider">Cancelar</button>
