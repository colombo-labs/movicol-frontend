...
> = {
  ...
  kennedy: { lat: 4.621, lng: -74.1522, label: "Kennedy, Bogotá" },
  bosa: { lat: 4.5963, lng: -74.1827, label: "Bosa, Bogotá" },
  engativa: { lat: 4.706, lng: -74.1106, label: "Engativá, Bogotá" },
  fontibon: { lat: 4.678, lng: -74.1463, label: "Fontibón, Bogotá" },
  teusaquillo: { lat: 4.6337, lng: -74.0833, label: "Teusaquillo, Bogotá" },
  "barrios unidos": {
    lat: 4.6603,
    lng: -74.0784,
    label: "Barrios Unidos, Bogotá",
  },
  "puente aranda": {
    lat: 4.6253,
    lng: -74.1172,
    label: "Puente Aranda, Bogotá",
  },
  "antonio narino": {
    lat: 4.5879,
    lng: -74.104,
    label: "Antonio Nariño, Bogotá",
  },
  "rafael uribe": {
    lat: 4.5666,
    lng: -74.1065,
    label: "Rafael Uribe Uribe, Bogotá",
  },
  "ciudad bolivar": {
    lat: 4.5423,
    lng: -74.1582,
    label: "Ciudad Bolívar, Bogotá",
  },
  "san cristobal": {
    lat: 4.5564,
    lng: -74.0818,
    label: "San Cristóbal, Bogotá",
  },
  "santa fe": { lat: 4.604, lng: -74.064, label: "Santa Fe, Bogotá" },
  "los martires": { lat: 4.6062, lng: -74.091, label: "Los Mártires, Bogotá" },
  tunjuelito: { lat: 4.5733, lng: -74.1325, label: "Tunjuelito, Bogotá" },
  usme: { lat: 4.481, lng: -74.1195, label: "Usme, Bogotá" },
  "portal norte": {
    lat: 4.759,
    lng: -74.0453,
    label: "Portal Norte TM, Bogotá",
  },
  "portal sur": { lat: 4.5955, lng: -74.157, label: "Portal Sur TM, Bogotá" },
  "portal americas": {
    lat: 4.6258,
    lng: -74.1758,
    label: "Portal Américas TM, Bogotá",
  },
  "portal eldorado": {
    lat: 4.6838,
    lng: -74.1238,
    label: "Portal El Dorado TM, Bogotá",
  },
  "portal 80": { lat: 4.718, lng: -74.1117, label: "Portal 80 TM, Bogotá" },
  "portal suba": {
    lat: 4.7551,
    lng: -74.0937,
    label: "Portal Suba TM, Bogotá",
  },
  "portal tunal": {
    lat: 4.5712,
    lng: -74.131,
    label: "Portal Tunal TM, Bogotá",
  },
  ...
  chia: { lat: 4.8637, lng: -74.054, label: "Chía, Cundinamarca" },
  zipaquira: { lat: 5.0224, lng: -73.9932, label: "Zipaquirá, Cundinamarca" },
  mosquera: { lat: 4.7088, lng: -74.233, label: "Mosquera, Cundinamarca" },
  funza: { lat: 4.717, lng: -74.2116, label: "Funza, Cundinamarca" },
  "el dorado": {
    lat: 4.7016,
    lng: -74.1469,
    label: "Aeropuerto El Dorado, Bogotá",
  },
  aeropuerto: {
    lat: 4.7016,
    lng: -74.1469,
  ...
  terminal: {
    lat: 4.6554,
    lng: -74.1178,
    label: "Terminal de Transporte, Bogotá",
  },
...
async function searchPhoton(query: string): Promise<GeoResult[]> {
  try {
    ...
    const BOG_LAT_MAX = 4.9;
    const BOG_LNG_MIN = -74.25;
    const BOG_LNG_MAX = -73.9;
    ...
    return filtered.slice(0, 5).map((f: any) => {
...
export async function geocodeAddress(query: string): Promise<GeoResult[]> {
  ...
  const zone =
    BOGOTA_ZONES[q] ||
    Object.values(BOGOTA_ZONES).find((_, idx) =>
      q.includes(Object.keys(BOGOTA_ZONES)[idx]),
    );
  if (zone) {
    // Find matching key for the zone
    const key =
      Object.keys(BOGOTA_ZONES).find((k) => BOGOTA_ZONES[k] === zone) || q;
    const zoneResult: GeoResult = {
      lat: zone.lat,
      lng: zone.lng,
      label: zone.label,
      type: "address",
